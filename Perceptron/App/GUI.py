...
class App(Frame):
    ...
    def train_algo(self):
        ...
        if learning_r == '':
            if iterations == '':
                choices_set = self.main.train()
            else:
                choices_set = self.main.train(int(iterations), 0)
        else:
            if iterations == '':
                choices_set = self.main.train(0, float(learning_r))
            else:
                choices_set = self.main.train(int(iterations), float(learning_r))
        self.set_img()
    ...
    def set_img(self):
        img = Image.open("/Users/juliadebecka/Documents/GitHub/NAI/Perceptron/Resources"
                                            "/errors_iter.png")
        tw = 0.5
    ...
    def open_data_to_learn(self):
        filepath = askopenfilename(initialdir="/Users/juliadebecka/Documents/GitHub/NAI/Perceptron/Resources/",
    ...
    def test_custom_vec(self):
        ...
        vec_name = self.option.get()
        temp = vec.split(",")
        floatList: [float] = []
        for argument in temp:
            floatList.append(float(argument))
        acc = self.main.test_custom(floatList, vec_name)
